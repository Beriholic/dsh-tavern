...
实测记录（2026-09-11）：现有 MuMu、DSHA 1.2.0-rc1、酒馆 v1.6.0。内置“进入 → 酒馆工作台”成功；模拟器自带浏览器直接打开 3088 复现上述报错，使用本次启动的完整 3080 认证地址后再点“酒馆工作台”，成功显示酒馆页面。本次验证限于认证与页面进入，不代表外部浏览器的模型生成和全部功能验证通过；推荐安装版本仍以上文为准。
