...
必须用外部浏览器时，从 DSHA「启动」页本次启动日志的「本机打开」取得**完整地址（包括 `?token=` 后的全部内容）**，在同一台安卓设备的浏览器打开，再点「酒馆工作台」。找不到完整地址或不方便复制时，使用上面的 DSHA 内置入口。详见 [浏览器认证与恢复步骤](docs/android-install.md#浏览器提示需要认证怎么办)。
