...
function plan() {
  ...
  const chat = timeline.apply({ chat: { id:'chat', messages:[], scriptState:null, mvu:{enabled:true} }, intent:{kind:'ensure'} }).chat
  return buildImportedConversation(chat, parseChatHistory(text), {operationId:'import-test'})
}
test('real host restores imported roles and turns without model execution', async () => {
  ...
  assert.deepEqual(session.deriveMessages().map(m=>m.content[0].text), ['opening','walk','arrived','rest','rested'])
  ...
  assert.equal(restored.deriveMessages().length,5)
})
...
test('consecutive roles form native rounds and retain every paragraph and the final state',async()=>{
 ...
 assert.deepEqual(session.deriveMessages().flatMap(m=>m.content.map(b=>b.text)),['opening\n\nopening continued'])
})
