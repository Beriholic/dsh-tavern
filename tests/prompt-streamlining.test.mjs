...
test('外部预设作用于前台游玩，后台与卡片 Agent 保持 DSH 原生上下文', () => {
  ...
  assert.match(startChat, /chat\.runtimePresetPath = str\(runtimePresetSnapshot && runtimePresetSnapshot\.presetPath\)/)
  const resolver = between(serverSource, 'async function resolveChatRuntimePreset', 'function compatibilityWorldBookMatch')
  assert.match(resolver, /return chat\.runtimePresetSnapshot && typeof chat\.runtimePresetSnapshot === 'object'/)
  assert.doesNotMatch(resolver, /runtimePresets\.fullSnapshot|updateChat/)
  ...
  assert.match(compatibility, /presetPath === '' \? createCleanCompatibilityPreset\(\) : snapshot\.compatibilityPreset/)
  assert.match(compatibility, /presetPath === '' \? \{\} : snapshot\.compatibilityPresetDocument/)
  ...
  assert.match(serverSource, /const activePresetSnapshot = groupOfMode\(chat\.mode\) === 'play' && chat\.runtimePresetSnapshot/)
...
test('人物卡基本信息固定为会话前缀，仅系统和历史后指令逐轮注入', () => {
  ...
  assert.match(buildSnapshot, /worldBook = await worldBooks\.bound\(chat.cardPath, card, chat\)/)
