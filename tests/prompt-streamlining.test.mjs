...
test('外部预设作用于前台游玩，后台与卡片 Agent 保持 DSH 原生上下文', () => {
  ...
  assert.match(compatibility, /presetPath === '' \? createCleanCompatibilityPreset\(\) : await readPreset\(presetPath\)/)
  assert.match(compatibility, /presetPath === '' \? \{\} : await readPresetDocument\(presetPath\)/)
